import { Node, Schema, Slice } from '@remirror/pm/model'

import { Mappable, Step, StepMap, StepResult } from '@remirror/pm/transform'
...
/// slice.
export class ReplaceAroundV2Step extends Step {
  ...
  invert(doc: Node) {
    ...
    return new ReplaceAroundV2Step(
      this.from,
  ...
  static fromJSON(schema: Schema, json: any) {
    if (
      ...
      throw new RangeError('Invalid input for ReplaceAroundV2Step.fromJSON')
    return new ReplaceAroundV2Step(
      json.from,
      json.to,
      json.gapFrom,
...
Step.jsonID('replaceAroundV2', ReplaceAroundV2Step)
