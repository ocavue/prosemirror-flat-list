...
function fixList(tr) {
	...
	for (const pos of joinable) if ((0, prosemirror_transform.canJoin)(tr.doc, pos)) tr.join(pos);
	const splitable = findBoundaries(ranges.next().value, tr.doc, isListSplitable);
	for (const pos of splitable) if ((0, prosemirror_transform.canSplit)(tr.doc, pos)) tr.split(pos);
	return tr;
}
/** @internal */
...
function findListsRange($from, $to = $from) {
	...
	while (range) {
		...
		range = new prosemirror_model.NodeRange($from, $to, range.depth - 1);
	}
	return null;
}
/** @internal */
...
function safeLift(tr, range) {
	const target = (0, prosemirror_transform.liftTarget)(range);
...
function moveOutOfCollapsed($pos, minDepth) {
	for (let depth = minDepth; depth <= $pos.depth; depth++) if (isCollapsedListNode($pos.node(depth)) && $pos.index(depth) >= 1) {
		...
		return prosemirror_state.TextSelection.near($before, -1);
	}
	return null;
}
/**
...
function dedentRange(range, tr, startBoundary, endBoundary) {
	...
	if (!endBoundary) {
		...
		range = new prosemirror_model.NodeRange(tr.doc.resolve($from.pos), tr.doc.resolve(endOfParent), depth);
		return dedentRange(range, tr, void 0, true);
	}
	if (range.startIndex === 0 && range.endIndex === range.parent.childCount && isListNode(range.parent)) return dedentNodeRange(new prosemirror_model.NodeRange($from, $to, depth - 1), tr);
...
function splitAndDedentRange(range, tr, splitIndex) {
	...
	if (range2 && range2.depth >= depth) {
		range2 = new prosemirror_model.NodeRange(range2.$from, range2.$to, depth);
...
function safeLiftRange(tr, range) {
	if (moveRangeSiblings(tr, range)) range = new prosemirror_model.NodeRange(tr.doc.resolve(range.$from.pos), tr.doc.resolve(range.$to.pos), range.depth);
	return safeLift(tr, range);
}
function moveRangeSiblings(tr, range) {
	...
	if (end < endOfParent) {
		...
		if (endIndex < parent.childCount && lastChild.canReplace(lastChild.childCount, lastChild.childCount, parent.content, endIndex, parent.childCount)) {
			tr.step(new prosemirror_transform.ReplaceAroundStep(end - 1, endOfParent, end, endOfParent, new prosemirror_model.Slice(prosemirror_model.Fragment.from(listType.create(null)), 1, 0), 0, true));
			return true;
		} else {
			tr.step(new prosemirror_transform.ReplaceAroundStep(end, endOfParent, end, endOfParent, new prosemirror_model.Slice(prosemirror_model.Fragment.from(listType.create(null)), 0, 0), 1, true));
			return true;
		}
	}
	return false;
...
function fixEndBoundary(range, tr) {
	if (range.endIndex - range.startIndex >= 2) range = new prosemirror_model.NodeRange(range.$to.doc.resolve(range.$to.posAtIndex(range.endIndex - 1, range.depth)), range.$to, range.depth);
	const contentRange = zoomInRange(range);
	if (contentRange) {
		fixEndBoundary(contentRange, tr);
		range = new prosemirror_model.NodeRange(tr.doc.resolve(range.$from.pos), tr.doc.resolve(range.$to.pos), range.depth);
	}
...
function dedentOutOfList(tr, range) {
	...
	if (!$start.parent.canReplace(startIndex, startIndex + 1, prosemirror_model.Fragment.from(listNode))) return false;
...
function indentNodeRange(range, tr) {
	...
	if (prevChild && isListNode(prevChild)) {
		...
		tr.step(new prosemirror_transform.ReplaceAroundStep(start - 1, end, start, end, new prosemirror_model.Slice(prosemirror_model.Fragment.from(listType.create(null)), 1, 0), 0, true));
		return true;
	}
	...
	if (startIndex === 0 && isParentListNode || isFirstChildListNode) {
		...
		tr.step(new prosemirror_transform.ReplaceAroundStep(start, end, start, end, new prosemirror_model.Slice(prosemirror_model.Fragment.from(listType.create(listAttrs)), 0, 0), 1, true));
		return true;
	}
	return false;
}
...
function joinTextblocksAround(tr, $cut, dispatch) {
	...
	let step = (0, prosemirror_transform.replaceStep)(tr.doc, beforePos, afterPos, prosemirror_model.Slice.empty);
	if (!step || step.from != beforePos || step instanceof prosemirror_transform.ReplaceStep && step.slice.size >= afterPos - beforePos) return false;
	if (dispatch) {
		tr.step(step);
		tr.setSelection(prosemirror_state.TextSelection.create(tr.doc, beforePos));
...
const joinCollapsedListBackward = (state, dispatch, view) => {
	...
	if (nodeBefore && nodeAfter && isListNode(nodeBefore) && nodeBefore.attrs.collapsed && nodeAfter.isBlock) {
		...
		tr.setSelection(prosemirror_state.TextSelection.near($insert));
...
function liftListContent(state, dispatch, $cursor) {
	...
	if (safeLift(tr, new prosemirror_model.NodeRange($cursor, tr.doc.resolve($cursor.end(listDepth)), listDepth))) dispatch(tr);
}
...
function createSplitListCommand() {
	return withAutoFixList((0, prosemirror_commands.chainCommands)(splitBlockNodeSelectionInListCommand, splitListCommand));
}
...
const splitBlockNodeSelectionInListCommand = (state, dispatch) => {
	...
	if (dispatch) {
		...
		tr.replace(cutPoint, cutPoint, new prosemirror_model.Slice(prosemirror_model.Fragment.fromArray([listType.create(), nextList]), 1, 1));
		const newSelection = prosemirror_state.TextSelection.near(tr.doc.resolve(cutPoint));
...
const splitListCommand = (state, dispatch) => {
	...
	if (indexInList === 0) if (parentEmpty) {
		...
		const range = $from.index(listParentDepth) === listParent.childCount - 1 ? new prosemirror_model.NodeRange($from, $listEnd, listParentDepth) : new prosemirror_model.NodeRange($from, $listEnd, listDepth);
		const tr = state.tr;
...
function doSplitList(state, listNode, dispatch) {
	...
	if (atEnd && attrs.collapsed) {
		if (dispatch) {
			...
			tr.insert(pos, createAndFill(listType, newAttrs));
			tr.setSelection(prosemirror_state.Selection.near(tr.doc.resolve(pos)));
			dispatch(tr.scrollIntoView());
		}
		return true;
	}
	...
	if (!(0, prosemirror_transform.canSplit)(tr.doc, $from.pos, 2, typesAfter)) return false;
...
* @public @group Commands
*/
const enterCommand = (0, prosemirror_commands.chainCommands)(protectCollapsed, createSplitListCommand());
/**
...
*
*/
const backspaceCommand = (0, prosemirror_commands.chainCommands)(protectCollapsed, prosemirror_commands.deleteSelection, joinListUp, joinCollapsedListBackward, prosemirror_commands.joinTextblockBackward, prosemirror_commands.selectNodeBackward);
/**
...
*
*/
const deleteCommand = (0, prosemirror_commands.chainCommands)(protectCollapsed, prosemirror_commands.deleteSelection, prosemirror_commands.joinTextblockForward, prosemirror_commands.selectNodeForward);
/**
...
function createWrapInListCommand(getAttrs) {
	const wrapInList = (state, dispatch) => {
		...
		let range = $from.blockRange($to);
		if (!range) return false;
		if (rangeAllowInlineContent(range) && isListNode(range.parent) && range.depth > 0 && range.startIndex === 0) range = new prosemirror_model.NodeRange($from, $to, range.depth - 1);
...
function wrappingListInputRule(regexp, getAttrs) {
	return new prosemirror_inputrules.InputRule(regexp, (state, match, start, end) => {
		const tr = state.tr;
		...
		const wrapping = (0, prosemirror_transform.findWrapping)(range, getListType(state.schema), newAttrs);
...
* @public @group Plugins
*/
var ListDOMSerializer = class ListDOMSerializer extends prosemirror_model.DOMSerializer {
	static nodesFromSchema(schema) {
		return {
			...prosemirror_model.DOMSerializer.nodesFromSchema(schema),
			list: (node) => listToDOM({
				node,
				nativeList: true,
				getMarkers: () => null
...
function createListClipboardPlugin(schema) {
	return new prosemirror_state.Plugin({ props: {
...
function createListEventPlugin() {
	return new prosemirror_state.Plugin({ props: { handleDOMEvents: { mousedown: (view, event) => handleListMarkerMouseDown({
		view,
		event
	}) } } });
}
...
exports.createDedentListCommand = createDedentListCommand;
exports.createIndentListCommand = createIndentListCommand;
exports.createListClipboardPlugin = createListClipboardPlugin;
exports.createListEventPlugin = createListEventPlugin;
exports.createListNodeView = createListNodeView;
exports.createListPlugins = createListPlugins;
exports.createListRenderingPlugin = createListRenderingPlugin;
exports.createListSpec = createListSpec;
exports.createMoveListCommand = createMoveListCommand;
exports.createParseDomRules = createParseDomRules;
exports.createSafariInputMethodWorkaroundPlugin = createSafariInputMethodWorkaroundPlugin;
exports.createSplitListCommand = createSplitListCommand;
exports.createToggleCollapsedCommand = createToggleCollapsedCommand;
exports.createToggleListCommand = createToggleListCommand;
exports.createUnwrapListCommand = createUnwrapListCommand;
exports.createWrapInListCommand = createWrapInListCommand;
exports.defaultAttributesGetter = defaultAttributesGetter;
exports.defaultListClickHandler = defaultListClickHandler;
exports.defaultMarkerGetter = defaultMarkerGetter;
exports.deleteCommand = deleteCommand;
exports.doSplitList = doSplitList;
exports.enterCommand = enterCommand;
exports.enterWithoutLift = enterWithoutLift;
exports.findCheckboxInListItem = findCheckboxInListItem;
exports.findListsRange = findListsRange;
exports.flatListGroup = flatListGroup;
exports.getListType = getListType;
exports.handleListMarkerMouseDown = handleListMarkerMouseDown;
exports.isCollapsedListNode = isCollapsedListNode;
exports.isListNode = isListNode;
exports.isListType = isListType;
exports.isListsRange = isListsRange;
exports.joinCollapsedListBackward = joinCollapsedListBackward;
exports.joinListElements = joinListElements;
exports.joinListUp = joinListUp;
exports.listInputRules = listInputRules;
exports.listKeymap = listKeymap;
exports.listToDOM = listToDOM;
exports.migrateDocJSON = migrateDocJSON;
exports.parseInteger = parseInteger;
exports.protectCollapsed = protectCollapsed;
exports.rangeToString = rangeToString;
exports.setSafeSelection = setSafeSelection;
exports.unwrapListSlice = unwrapListSlice;
exports.wrappingListInputRule = wrappingListInputRule;