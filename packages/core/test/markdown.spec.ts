...
describe('markdown helper', () => {
  ...
  it('can parse task list', () => {
    ...
    expect(output.toJSON()).toEqual(
      t
        .doc(t.uncheckedTaskList(t.p('A1')), t.checkedTaskList(t.p('A2')))
        .toJSON(),
    )
  })

  it('can parse ordered list', () => {
    const t = setupTestingEditor()
    const output = t.markdown`
      1. A1
      2. A2
    `
    expect(output.toJSON()).toEqual(
      t.doc(t.orderedList(t.p('A1')), t.orderedList(t.p('A2'))).toJSON(),
    )
  })
})
